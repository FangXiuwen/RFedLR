...
def partition_data(dataset, datadir, partition, num_classes, num_users, dirichlet_beta=1.0):
    if dataset == 'cifar10':
...
def generate_proxy_data_indexs(dataset,datadir,size):
    if dataset == 'cifar10':
        X_train, y_train, X_test, y_test = load_cifar10_data(datadir)
    if dataset =='cifar100':
        X_train, y_train, X_test, y_test = load_cifar100_data(datadir)
...
#-------Origin dataloader--------
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_type=None, noise_rate=0):
#-------Origin dataloader--------
    if dataset == 'cifar10':
        ...
        train_ds = Cifar10FL(datadir, dataidxs=dataidxs, train=True, transform=transform_train, noise_type=noise_type, noise_rate=noise_rate)
        transform_test = transforms.Compose([
            transforms.Resize((224, 224)),
            transforms.ToTensor(),
            transforms.Normalize(mean=[x / 255.0 for x in [125.3, 123.0, 113.9]], std=[x / 255.0 for x in [63.0, 62.1, 66.7]])
        ])
        test_ds = Cifar10FL(datadir, train=False, transform=transform_test)
    if dataset =='cifar100':
        transform_train = transforms.Compose([
            transforms.ToPILImage(),
        ...
        train_ds = Cifar100FL(datadir, dataidxs=dataidxs, train=True, transform=transform_train, noise_type=noise_type, noise_rate=noise_rate)
        transform_test = transforms.Compose([
            transforms.ToPILImage(),
            transforms.Resize((224, 224)),
            transforms.ToTensor(),
        ...
        test_ds = Cifar100FL(datadir, train=False, transform=transform_test)
    ...
    test_dl = data.DataLoader(dataset=test_ds, batch_size=test_bs, drop_last=True, shuffle=False,num_workers=4)
    return train_dl, test_dl, train_ds, test_ds

#-------Origin dataloader--------
def get_proxy_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_type=None, noise_rate=0):
#-------Origin dataloader--------
    if dataset == 'cifar10':
        transform_train = transforms.Compose([
            transforms.ToTensor(),
    ...
    if dataset == 'tinyimagenet':
        ...
        if dataidxs is None:
            ...
            train_ds = datasets.ImageFolder(datadir+'/train', transform=transform_train)
            train_ds = torch.utils.data.Subset(train_ds, dataidxs)
    train_dl = data.DataLoader(dataset=train_ds, batch_size=train_bs, drop_last=True, shuffle=False)
    test_dl = data.DataLoader(dataset=test_ds, batch_size=test_bs, drop_last=True, shuffle=False)
    return train_dl, test_dl, train_ds, test_ds

if __name__ =='__main__':
    logger = init_logs()
    public_data_indexs = generate_proxy_data_indexs(dataset='cifar100',datadir='./cifar_100',size=5000)
