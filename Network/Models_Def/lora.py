...
class LoRA_ViT_timm(nn.Module):
    ...
    def freeze_lora_parameters(self, freeze_a: bool = False, freeze_b: bool = False):
        """
        Freeze or unfreeze LoRA matrices A and B.
        
        Args:
            freeze_a (bool): If True, freeze A matrices, if False, unfreeze them
            freeze_b (bool): If True, freeze B matrices, if False, unfreeze them
        """
        # Handle A matrices
        for w_A in self.w_As:
            w_A.weight.requires_grad = not freeze_a
        # Handle B matrices
        for w_B in self.w_Bs:
            w_B.weight.requires_grad = not freeze_b
    
    def get_lora_state(self) -> dict:
        """
        Get the current state of LoRA matrices (frozen or not).
        
        Returns:
            dict: Dictionary containing the frozen state of matrices A and B
        """
        a_frozen = not self.w_As[0].weight.requires_grad if self.w_As else True
        b_frozen = not self.w_Bs[0].weight.requires_grad if self.w_Bs else True
        return {
            "a_matrices_frozen": a_frozen,
            "b_matrices_frozen": b_frozen
        }
